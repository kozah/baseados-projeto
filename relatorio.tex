...
\subsection*{Query 4}
Quais filmes do diretor do filme \textbf{F} possuem \textbf{X} ou \textbf{Y} como atores, com as respectivas durações em ordem crescente?

Com \textbf{F} = onto:filme\_her, \textbf{X} = onto:amy\_addams e \textbf{Y} = onto:tim\_szczepanski.
\begin{verbatim}
...
\subsection*{Query 6}
Quais diretores dirigiram algum filme em um ano entre os anos \textbf{N\textsubscript{1}} e \textbf{N\textsubscript{2}},  em que os atores \textbf{X} e \text{Y} aparecem, do mais antigo para o mais novo?
\begin{verbatim}
...
\subsection*{Query 7}
Quais pares formados por uma uma atriz e um ator atuaram juntos em algum filme de duração entre \textbf{M1} e \textbf{M2}?
\begin{verbatim}
...
\subsection*{Query 8}
Qual o diretor que mais dirigiu filmes em que aparece o ator de primeiro nome \textbf{X\textsubscript{p}} e último nome \textbf{X\textsubscript{u}}?

Com \textbf{X\textsubscript{p}} = Vincent, \textbf{X\textsubscript{u}} = Coppola
\begin{verbatim}
SELECT ?Diretor
WHERE
{
  ...
  ?pessoa onto:atuaEm ?filme.
  ?pessoa onto:temPrimeiroNome ?XP.
  ?pessoa onto:temSobrenome ?XU.
  ?Diretor onto:dirige ?filme.
}
GROUP BY ?Diretor
...
\subsection*{Query 9}
Qual o filme mais antigo em que o ator \textbf{X} atuou, em que ano foi lançado e qual era seu personagem? Havendo empate, devolver todos.
Com \textbf{X} = onto:alan\_rachins.
\begin{verbatim}
...
\subsection*{Query 10}
Qual o filme mais longo dirigido por \textbf{D}, e qual a sua duração? Havendo empate, devolver todos da mesma duração.

Para \textbf{D} = onto:ethan\_coen.
\begin{verbatim}
SELECT ?Filme ?Duracao
WHERE
{
